...
RUN curl -OL https://github.com/apache/incubator-predictionio/archive/v${PIO_VERSION}-incubating.tar.gz \
    && tar -xvzf v${PIO_VERSION}-incubating.tar.gz -C / \
    && rm v${PIO_VERSION}-incubating.tar.gz \
    && cd incubator-predictionio-${PIO_VERSION}-incubating \
    && ./make-distribution.sh

RUN tar zxvf /incubator-predictionio-${PIO_VERSION}-incubating/PredictionIO-${PIO_VERSION}-incubating.tar.gz -C /
RUN rm -r /incubator-predictionio-${PIO_VERSION}-incubating
...
RUN pip install setuptools wheel predictionio datetime
